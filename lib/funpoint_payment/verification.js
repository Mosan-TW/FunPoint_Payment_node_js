 ...
 */
const FunPointError = require('./error.js');
const fs = require('fs');
        ...
        Object.keys(params).forEach(function (p_name) {
           ...
           switch (p_type) {
               case 'String':
                   ...
                   if (mat === null){
                       throw new FunPointError.FunPointInvalidParam(`Wrong format of param ${p_name} or length exceeded.`);
                   }
                   break;
               case 'Opt':
                   ...
                   if (mat_opt === false){
                       throw new FunPointError.FunPointInvalidParam(`Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_opt}).`);
                   }
                   break;
               case 'Int':
                   ...
                   switch (mode){
                       case 'BETWEEN':
                           if (val < min || val > max){
                               throw new FunPointError.FunPointInvalidParam(`Value of ${p_name} should be between ${min} and ${max}.`);
                           }
                           break;
                       case 'GE':
                           if (val < min){
                               throw new FunPointError.FunPointInvalidParam(`Value of ${p_name} should be greater than or equal to ${min}.`);
                           }
                           break;
                       case 'LE':
                           if (val > max){
                               throw new FunPointError.FunPointInvalidParam(`Value of ${p_name} should be less than or equal to ${max}.`);
                           }
                           break;
                       case 'EXCLUDE':
                           if (val >= max && val <= max){
                               throw new FunPointError.FunPointInvalidParam(`Value of ${p_name} can NOT be between ${min} and ${max}..`);
                           }
                           break;
                       default:
                           throw new FunPointError.FunPointInvalidParam(`Unexpected integer verification mode for parameter ${p_name}: ${mode}.`);
                   }
                   break;
               case 'DepOpt':
                   ...
                   if (aval_dopt === null && pattern['Opt'][parent_param].includes(parent_val) === false){
                       throw new FunPointError.FunPointInvalidParam(`Cannot find available option of [${p_name}] by related param [${parent_param}](Value: ${parent_val}.`);
                   } else if (aval_dopt.constructor === Array){
                       if (!aval_dopt.includes(params[p_name])){
                           throw new FunPointError.FunPointInvalidParam(`Unexpected option of param ${p_name} (${params[p_name]}). Available option: (${aval_dopt}).`);
                       }
                   }
                   break;
               default:
...
class AioCheckOutParamVerify extends PaymentVerifyBase{
    ...
    verify_aio_payment_param(params){
        if (params.constructor === Object){
            ...
            // if (params === {}){
            //     throw new FunPointError.FunPointInvalidParam(`Parameter object is empty.`);
            // }
            ...
            basic_param.forEach(function (pname) {
               if (input_param.indexOf(pname, 0) === -1){
                   throw new FunPointError.FunPointInvalidParam(`Lack required param ${pname}`);
               }
            });
               ...
               if (related_required_param !== undefined && related_required_param !== []){
                   related_required_param.forEach(function (e) {
                       if (!Object.keys(params).includes(e)){
                           throw new FunPointError.FunPointInvalidParam(`Lack required parameter [${e}] when [${pa}] is set to [${val}].`);
                       }
                   });
               }
            });
    ...
    verify_aio_inv_param(params){
        if (params.constructor === Object){
            ...
            Object.keys(params).forEach(function (keys) {
               if (params[keys] === null){
                   throw new FunPointError.FunPointInvalidParam(`Parameter value cannot be null.`);
               }
            });
            // 1. 比對欄位是否缺乏
            ...
            inv_param.forEach(function (pname) {
                if (input_param.indexOf(pname, 0) === -1){
                    throw new FunPointError.FunPointInvalidParam(`Lack required invoice param ${pname}`);
                }
            });
            input_param.forEach(function (pname) {
                if (inv_param.indexOf(pname, 0) === -1){
                    throw new FunPointError.FunPointInvalidParam(`Unexpected parameter in Invoice parameters ${pname}`);
                }
            });
            ...
            // if (param_diff !== []){
            //     throw new FunPointError.FunPointInvalidParam(`Lack required invoice param ${param_diff}`);
            // }
            // let unexp_param = Object.keys(params) - inv_param_names;
            // if (unexp_param !== []){
            //     throw new FunPointError.FunPointInvalidParam(`Unexpected parameter in Invoice parameters ${unexp_param}`);
            // }
            ...
            if (params['CarruerType'] === '1'){
                if (params['CustomerID'] === ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CustomerID] can not be empty when [CarruryType] is 1.`);
                }
            } // [CustomerID]不為空 => CarruerType 不能為空
            else if (params['CarruerType'] === ''){
                if (params['CustomerID'] !== ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CarruerType] can not be empty when [CustomerID] is not empty.`);
                }
            }
            // b 列印註記[Print]為1 => CustomerName, CustomerAddr 不能為空
            if (params['Print'] === '1'){
                if (params['CustomerName'] === '' && params['CustomerAddr'] === ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CustomerName] and [CustomerAddr] can not be empty when [Print] is 1.`);
                }
                if (params['CustomerID'] !== ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[Print] can not be '1' when [CustomerID] is not empty.`);
                }
            }
            // c CustomerPhone和CustomerEmail至少一個要有值
            if (params['CustomerPhone'] === '' && params['CustomerEmail'] === ''){
                throw new FunPointError.FunPointInvoiceRuleViolate(`[CustomerPhone] and [CustomerEmail] can not both be empty.`);
            }
            ...
            if (params['TaxType'] === '2'){
                if (params['ClearanceMark'] !== '1' && params['ClearanceMark'] !== '2'){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[ClearanceMark] has to be 1 or 2 when [TaxType] is 2.`);
                }
                if (!params['InvoiceItemTaxType'].includes('2')){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[InvoiceItemTaxType] must contain at lease one '2' when [TaxType] is 2.`);
                }
            }
            ...
            if (params['CustomerIdentifier'] !== ''){
                if (params['CarruerType'] === '1' || params['CarruerType'] === '2'){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CarruerType] Cannot be 1 or 2 when [CustomerIdentifier] is given.`);
                }
            }
            ...
            if (params['CarruerType'] === '' || params['CarruerType'] === '1'){
                if (params['CarruerNum'] !== ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CarruerNum] must be empty when [CarruerType] is empty or 1.`);
                }
            } else if (params['CarruerType'] === '2'){
                if (params['CarruerNum'].match(new RegExp(/[A-Za-z]{2}[0-9]{14}/)) === null){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CarruerNum] must be 2 alphabets and 14 numbers when [CarruerType] is 2.`);
                }
            } else if (params['CarruerType'] === '3'){
                if (params['CarruerNum'].match(new RegExp(/^\/[A-Za-z0-9\s+-]{7}$/)) === null){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[CarruerNum] must start with '/' followed by 7 alphabet and number characters when [CarruerType] is 3.`);
                }
            } else {
                throw new FunPointError.FunPointInvoiceRuleViolate(`Unexpected value in [CarruerType].`);
            }
            // [CarruerType]有值時，Print必須有為0
            if (params['CarruerType'] !== '' && params['Print'] !== '0'){
                throw new  FunPointError.FunPointInvoiceRuleViolate(`[Print] must be 0 when [CarruerType] has value.`);
            }
            // Donation = 1 => LoveCode不能為空, Print = 0
            if (params['Donation'] === '1'){
                if (params['LoveCode'] === ''){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[LoveCode] cannot be empty when [Donation] is 1.`);
                }
                if (params['Print'] !== '0'){
                    throw new FunPointError.FunPointInvoiceRuleViolate(`[Print] must be 0 when [Donation] is 1.`);
                }
            }
            ...
            if (params['InvoiceItemName'] === ''){
                throw new FunPointError.FunPointInvoiceRuleViolate(`[InvoiceItemName] is empty.`);
            } else {
                if (params['InvoiceItemName'].includes('|')){
                    ...
                    item_params.forEach(function (param_name) {
                        ...
                        if (params[param_name].match(new RegExp(/(\|\||^\||\|$)/)) !== null){
                            throw new FunPointError.FunPointInvoiceRuleViolate(`[${param_name}] contains empty value.`);
                        }
                        let p_cnt = params[param_name].split('|').length;
                        if (item_cnt !== p_cnt){
                            throw new FunPointError.FunPointInvoiceRuleViolate(`Count of item info [${param_name}] (${p_cnt}) not match count from [InvoiceItemName] (${item_cnt}).`);
                        }
                    });
                    ...
                    if (vio_tax_t === []){
                        throw new FunPointError.FunPointInvoiceRuleViolate(`Illegal [InvoiceItemTaxType]: ${vio_tax_t}`);
                    }
                    if (params['TaxType'] === '9'){
                        if (!item_tax.includes('1')){
                            throw new FunPointError.FunPointInvoiceRuleViolate(`[InvoiceItemTaxType] must contain at lease one '1'.`);
                        }
                        if (item_tax.includes('2') && item_tax.includes('3')){
                            throw new FunPointError.FunPointInvoiceRuleViolate(`[InvoiceItemTaxType] cannot contain 2 and 3 at the same time.`);
                        }
                    }
                } else {
                    // 沒有管線 => 逐一檢查後4項有無管線
                    item_params.forEach(function (param_name) {
                        if (params[param_name].includes('|')){
                            throw new FunPointError.FunPointInvoiceRuleViolate(`Item info [${param_name}] contain pipeline delimiter but there's only one item in param [InvoiceItemName].`);
                        }
                    });
                }
            }
